...
public class SkinCMD extends BaseCommand {
  ...
  public SkinCMD(Core instance) {
    ...
    instance
        ...
        .registerStaticCompletion(
            "variants", List.of("civilian", "guard", "participant", "tux", "original"));
              ...
              if (optionalSkins.isPresent()) {
                ...
                if (skins.size() > 0) {
                  ...
                  while (playerIter.hasNext()) {
                    ...
                    if (skin != null) {
                      Bukkit.getScheduler()
                          .runTask(
                              ...
                              task -> {
                                SkinCMD.skinSwapper(
                                    nextPlayer, skin.getValue(), skin.getSignature()); // Swap the
                                // Player's
                                // skin
                              });
                    }
                    ...
                    if (skinIndex > skins.size() - 1) skinIndex = 0;
                  }
                }
              }
              return false;
  ...
  public void changeSkinOther(
      CommandSender sender,
      String playerTarget,
      String skinSourcePlayer,
            ...
            if (player != null && player.isOnline()) {
              ...
              if (id != null) {
                if (variant.equalsIgnoreCase("original")) {
                  ...
                  SkinToolApi.getElseComputeSkins(id)
                      .whenComplete(
                          (skins, exception) -> {
                            if (exception != null) {
                              sender.sendMessage(
                                  "Command ended exceptionally: " + exception.getMessage());
                              exception.printStackTrace();
                            } else {
                              var skin =
                                  skins.stream()
                                      .filter(s -> s.getName().equalsIgnoreCase(variant))
                                      .findFirst();
                              if (skin.isPresent()) {
